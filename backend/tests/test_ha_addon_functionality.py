...
class TestHAAddonEnvironment:
    ...
    def test_laro_ha_addon_detection(self):
        """Test LARO_HA_ADDON environment variable detection"""
        # Test when HA addon mode is enabled
        with patch.dict(os.environ, {'LARO_HA_ADDON': 'true'}):
            assert os.getenv('LARO_HA_ADDON') == 'true'

        # Test when HA addon mode is disabled
        with patch.dict(os.environ, {'LARO_HA_ADDON': 'false'}, clear=True):
            assert os.getenv('LARO_HA_ADDON') == 'false'
    ...
    def test_database_url_configuration(self):
        ...
        expected_url = "postgresql://laro:laro@127.0.0.1:5432/laro"
...
class TestDebugRouter:
    ...
    def test_is_debug_enabled_ha_addon(self):
        ...
        with patch.dict(os.environ, {'LARO_HA_ADDON': 'true', 'DEBUG_MODE': 'false'}):
            assert is_debug_enabled() is True

        # Test with debug mode enabled
        with patch.dict(os.environ, {'LARO_HA_ADDON': 'false', 'DEBUG_MODE': 'true'}):
            assert is_debug_enabled() is True

        # Test with both disabled
        with patch.dict(os.environ, {'LARO_HA_ADDON': 'false', 'DEBUG_MODE': 'false'}):
