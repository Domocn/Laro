...
def get_allowed_redirect_uris(provider: str) -> set:
    """Get the set of allowed redirect URIs for a provider"""
    return {
        f"{OAUTH_REDIRECT_BASE_URL}/oauth/callback/{provider}",
        f"laro://oauth/callback/{provider}",
    }

def validate_redirect_uri(redirect_uri: str, provider: str) -> str:
    """Validate redirect_uri against allowlist, return validated URI"""
    allowed = get_allowed_redirect_uris(provider)
    if redirect_uri not in allowed:
        raise HTTPException(status_code=400, detail="Invalid redirect_uri")
    return redirect_uri

async def validate_oauth_state(state: str | None, expected_provider: str) -> dict:
    """Validate and consume OAuth state token. Raises on invalid/expired/wrong provider."""
    if not state:
        raise HTTPException(status_code=400, detail="Missing state parameter")

    state_doc = await oauth_state_repository.find_and_delete(state)
    if not state_doc:
        raise HTTPException(status_code=400, detail="Invalid state parameter")

    # Verify provider matches
    if state_doc.get("provider") != expected_provider:
        raise HTTPException(status_code=400, detail="State provider mismatch")

    # Verify state is not expired (10 minute max)
    try:
        created_at = datetime.fromisoformat(state_doc["created_at"])
        # Strip tzinfo for comparison (asyncpg-compatible naive datetimes)
        ...
        pass  # If created_at is missing/malformed, allow (backward compat)
...
async def get_google_auth_url(redirect_uri: str = Query(None)):
    ...
    if not redirect_uri:
        ...
        redirect_uri = validate_redirect_uri(redirect_uri, "google")
...
async def google_callback(data: OAuthCallback):
    ...
    state_doc = await validate_oauth_state(data.state, "google")

    # Use redirect_uri stored with state (trusted), fall back to client-provided (validated)
    redirect_uri = state_doc.get("redirect_uri") or (
        validate_redirect_uri(data.redirect_uri, "google") if data.redirect_uri
        else f"{OAUTH_REDIRECT_BASE_URL}/oauth/callback/google"
    )
...
async def get_github_auth_url(redirect_uri: str = Query(None)):
    ...
    if not redirect_uri:
        ...
        redirect_uri = validate_redirect_uri(redirect_uri, "github")
...
async def github_callback(data: OAuthCallback):
    ...
    state_doc = await validate_oauth_state(data.state, "github")

    # Use redirect_uri stored with state (trusted), fall back to client-provided (validated)
    redirect_uri = state_doc.get("redirect_uri") or (
        validate_redirect_uri(data.redirect_uri, "github") if data.redirect_uri
        else f"{OAUTH_REDIRECT_BASE_URL}/oauth/callback/github"
    )
