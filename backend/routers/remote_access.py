...
# Cloud relay URL from environment (don't hardcode infrastructure details)
CLOUD_RELAY_URL = os.getenv("CLOUD_RELAY_URL", "wss://laro.food/api/v1/remote/relay")
...
def check_rate_limit(client_ip: str) -> bool:
    """Check if client IP is rate limited. Returns True if allowed."""
    now = time.time()
    # Clean old entries
...
async def create_remote_instance(
    ...
    instance_id = f"laro_{secrets.token_hex(8)}"
...
async def delete_remote_instance(
    ...
    if instance_id in active_relays:
        try:
            ...
            logger.debug(f"Error closing relay for {instance_id}: {e}")
...
@router.post("/link")
async def link_to_cloud(request: Request, data: LinkInstanceRequest):
    """
    Called by self-hosted Laro to link to a cloud account using a linking code.
    Returns connection credentials for the relay.
    Rate limited to prevent brute force attacks on linking codes.
    """
    # Rate limit by IP
    client_ip = request.client.host if request.client else "unknown"
    if not check_rate_limit(client_ip):
        logger.warning(f"Rate limit exceeded for /link from {client_ip}")
...
async def client_relay(websocket: WebSocket, instance_id: str, token: str):
    ...
    except (jwt.InvalidTokenError, jwt.DecodeError, jwt.ExpiredSignatureError, KeyError) as e:
        logger.warning(f"Invalid JWT in client_relay: {type(e).__name__}")
        await websocket.close(code=4001)
        return
