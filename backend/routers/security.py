...
try:
    ...
    async def send_account_deletion_email(*args, **kwargs): return False
    async def send_account_deleted_notification(*args, **kwargs): return False
...
async def confirm_password_reset(data: PasswordResetConfirm):
    ...
    async with pool.acquire() as conn:
        ...
        """
        row = await conn.fetchrow(query, data.token, now)
...
# =============================================================================
# ACCOUNT DELETION (GDPR COMPLIANCE - NO LOGIN REQUIRED)
# =============================================================================

@router.post("/account-deletion/request")
async def request_account_deletion(data: AccountDeletionRequest, background_tasks: BackgroundTasks):
    """
    Request account deletion without being logged in (GDPR compliance).
    Sends a confirmation email with a deletion link.
    This allows users to delete their data even if they forgot their password.
    """
    user = await user_repository.find_by_email(data.email)
    ...
    if not user:
        return {
            "message": "If an account exists with this email, you will receive a deletion confirmation link.",
            "success": True
        }

    # Generate deletion token (24 hour expiry)
    token = secrets.token_urlsafe(32)
    ...
    # Send email if configured
    if is_email_configured():
        background_tasks.add_task(send_account_deletion_email, user["email"], token)
        return {
            "message": "If an account exists with this email, you will receive a deletion confirmation link.",
            "success": True
        }
...
async def confirm_account_deletion(data: AccountDeletionConfirm, background_tasks: BackgroundTasks):
    """
    ...
    try:
        async with pool.acquire() as conn:
            ...
            # Delete cookbooks
            await conn.execute("DELETE FROM cookbooks WHERE user_id = $1", user_id)
            # Delete meal plans
            await conn.execute("DELETE FROM meal_plans WHERE user_id = $1", user_id)
            # Delete shopping lists
            await conn.execute("DELETE FROM shopping_lists WHERE user_id = $1", user_id)
            # Delete pantry items
            await conn.execute("DELETE FROM pantry_items WHERE user_id = $1", user_id)
            # Delete sessions
            await conn.execute("DELETE FROM sessions WHERE user_id = $1", user_id)
            # Delete TOTP secrets
            await conn.execute("DELETE FROM totp_secrets WHERE user_id = $1", user_id)
            # Delete share links
            await conn.execute("DELETE FROM share_links WHERE user_id = $1", user_id)
            # Delete friends
            await conn.execute("DELETE FROM friends WHERE user_id = $1 OR friend_id = $1", user_id)
            # Delete the user
    ...
    if is_email_configured():
        background_tasks.add_task(send_account_deleted_notification, user_email)
