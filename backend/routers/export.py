...
EXPORT_DIR = Path(tempfile.gettempdir()) / "laro_exports"
...
async def export_user_data(
    ...
    if data.format == "json":
        # JSON export
        file_name = f"laro_export_{export_id}.json"
        ...
        zip_name = f"laro_export_{export_id}.zip"
...
async def download_export(
    ...
    json_path = EXPORT_DIR / f"laro_export_{export_id}.json"
    zip_path = EXPORT_DIR / f"laro_export_{export_id}.zip"

    if json_path.exists():
        return FileResponse(
            path=str(json_path),
            filename=f"laro_export_{datetime.now().strftime('%Y%m%d')}.json",
            media_type="application/json"
        )
    elif zip_path.exists():
        return FileResponse(
            path=str(zip_path),
            filename=f"laro_export_{datetime.now().strftime('%Y%m%d')}.zip",
...
async def gdpr_export_my_data(
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        ...
        try:
            ...
            # Optional data - continue if not available
            pass
