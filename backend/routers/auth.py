...
try:
    ...
    async def send_verification_email(*args, **kwargs): return False
...
class UserCreateExtended(BaseModel):
    ...
    def validate_name(cls, v: str) -> str:
        from models import _validate_human_name
...
async def get_password_policy() -> dict:
    """Get the current password policy from system settings"""
    settings = await system_settings_repository.get_settings()
    return {
...
async def password_policy():
    """Public endpoint to get current password requirements (no auth needed)"""
...
async def login(user: LoginWithTOTP, request: Request, background_tasks: BackgroundTasks):
    ...
    if not db_user.get("oauth_only") and not db_user.get("email_verified", False):
        # Return 403 with special error code for unverified email
        raise HTTPException(
            status_code=403,
            detail={
    ...
    # Check if user's existing name fails validation (legacy garbage names)
    name_update_required = False
    try:
        from models import _validate_human_name
        _validate_human_name(db_user.get("name", ""))
    except (ValueError, Exception):
        name_update_required = True
...
async def verify_email(data: VerifyEmailRequest, request: Request):
    ...
    db_user = await user_repository.find_by_verification_token(data.token)

    if not db_user:
        raise HTTPException(status_code=400, detail="Invalid or expired verification token")

    # Check if token is expired
    expires = db_user.get("email_verification_expires")
    now = datetime.now(timezone.utc).replace(tzinfo=None)
    if expires and expires < now:
        raise HTTPException(status_code=400, detail="Verification token has expired. Please request a new one.")

    # Mark email as verified and clear token
    await user_repository.update_user(db_user["id"], {
        "email_verified": True,
    ...
    if _debug_available:
        log_auth_event("EMAIL_VERIFIED", email=db_user["email"], ip_address=ip_address, success=True)
...
async def resend_verification(data: ResendVerificationRequest, request: Request, background_tasks: BackgroundTasks):
    """Resend email verification link"""
    ip_address = request.client.host if request.client else None

    # Rate limiting: Check recent verification emails sent
    # Using login_attempts table to track resend attempts
    now = datetime.now(timezone.utc).replace(tzinfo=None)
    cutoff = now - timedelta(hours=1)

    # Count recent resend attempts (we use action type to distinguish)
    recent_attempts = await login_attempt_repository.count_recent_by_email_and_action(
        data.email, "resend_verification", cutoff
    )
    ...
    # Find user by email
    db_user = await user_repository.find_by_email(data.email)

    # Record the resend attempt regardless of whether user exists
    await login_attempt_repository.create({
        "id": str(uuid.uuid4()),
        "email": data.email,
    ...
    if is_email_configured():
        background_tasks.add_task(send_verification_email, db_user["email"], db_user["name"], verification_token)

    if _debug_available:
        log_auth_event("VERIFICATION_RESENT", email=data.email, ip_address=ip_address, success=True)
