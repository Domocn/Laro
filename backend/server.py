...
# Configure laro.* loggers to inherit from root
for logger_name in ['laro', 'laro.auth', 'laro.db', 'laro.api', 'laro.websocket',
                    'laro.ai', 'laro.recipes', 'laro.security', 'laro.cache', 'laro.celery']:
    laro_logger = logging.getLogger(logger_name)
    laro_logger.setLevel(log_level)
...
class StartupState:
    ...
    def __init__(self):
        ...
        self.firebase_status: str = "unchecked"
...
async def lifespan(app: FastAPI):
    ...
    # Check Firebase credentials (non-blocking)
    try:
        from services.notifications import _get_fcm_credentials
        creds = _get_fcm_credentials()
        if creds:
            logger.info("Firebase credentials loaded - push notifications ready")
            startup_state.firebase_status = "ready"
        else:
            logger.warning("Firebase credentials not configured - push notifications disabled")
            startup_state.firebase_status = "not_configured"
    except Exception as e:
...
async def websocket_endpoint(websocket: WebSocket, token: str = None):
    ...
    1. First-message auth (preferred): Connect without token, send {"type": "auth", "token": "<jwt>"} as first message
    2. Query param auth (legacy): /ws?token=<jwt_token>
    """
    ...
    if not token:
        # First-message auth: accept connection, wait for auth message
        await websocket.accept()
        already_accepted = True
        try:
            auth_data = await asyncio.wait_for(websocket.receive_json(), timeout=10.0)
            if auth_data.get("type") != "auth" or not auth_data.get("token"):
                log_ws_event("AUTH_FAILED", error="Invalid auth message format")
                await websocket.close(code=4001)
                return
            token = auth_data["token"]
        except (asyncio.TimeoutError, Exception) as e:
            log_ws_event("AUTH_FAILED", error=f"Auth timeout or error: {str(e)}")
            await websocket.close(code=4001)
            return
    ...
    connection_id = await ws_manager.connect(websocket, user_id, household_id, already_accepted=already_accepted)
...
# Hardcode "static" subdir relative to server.py — immune to env var overrides
