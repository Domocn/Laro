...
class Settings:
    def __init__(self):
        ...
        self.database_url: str = os.getenv("DATABASE_URL", "postgresql://laro:laro@localhost:5432/laro")
        ...
        if os.getenv("RAILWAY_ENVIRONMENT"):
            cors_default = "https://laro.food"
        else:
            cors_default = "http://localhost:3000,http://localhost:3001,https://laro.food"
        self.cors_origins: str = os.getenv("CORS_ORIGINS", cors_default)
        ...
        # Auto-detect: serve frontend if explicitly enabled OR if index.html exists in static dir
        serve_env = os.getenv("SERVE_FRONTEND", "auto")
        if serve_env.lower() == "auto":
            self.serve_frontend: bool = os.path.exists(os.path.join(self.static_files_dir, "index.html"))
        else:
            self.serve_frontend: bool = serve_env.lower() == "true"
