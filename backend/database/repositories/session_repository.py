...
class LoginAttemptRepository(BaseRepository):
    ...
    async def count_recent_by_email_and_action(
        self,
        email: str,
        action: str,
        since: str
    ) -> int:
        """Count attempts by email since a specific time (for rate limiting resend verification).
        Uses the success=True field to track different action types - a simple approach
        since resend attempts are marked as success=True when recorded for rate limiting."""
        pool = await self._get_db()

        # For rate limiting resend verification, count all successful attempts since the cutoff
        query = """
            SELECT COUNT(*) FROM login_attempts
            WHERE email = $1 AND success = TRUE AND timestamp >= $2
        """
