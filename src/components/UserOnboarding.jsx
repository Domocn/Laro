...
export const UserOnboarding = () => {
  ...
  useEffect(() => {
    ...
    const checkOnboarding = async () => {
      ...
      const hasSeenOnboarding = localStorage.getItem(`laro_onboarding_${user?.id}`);
  ...
  const handleSkip = () => {
    localStorage.setItem(`laro_onboarding_${user?.id}`, 'skipped');
    setOpen(false);
  };
  ...
  const handleNext = async () => {
    if (currentStep === ONBOARDING_STEPS.length - 1) {
      ...
      try {
        ...
        localStorage.setItem(`laro_onboarding_${user?.id}`, 'completed');
        setOpen(false);
      } catch (error) {
            ...
            >
              ...
              <div className="mt-6 space-y-3">
                <div className="flex items-start gap-3 p-3 bg-cream-subtle dark:bg-muted rounded-xl">
                  <Heart className="w-5 h-5 text-laro flex-shrink-0 mt-0.5" />
                  <div>
                ...
                <div className="flex items-start gap-3 p-3 bg-cream-subtle dark:bg-muted rounded-xl">
                  <Settings className="w-5 h-5 text-laro flex-shrink-0 mt-0.5" />
                  <div>
                ...
                <div className="flex items-start gap-3 p-3 bg-cream-subtle dark:bg-muted rounded-xl">
                  <Sparkles className="w-5 h-5 text-laro flex-shrink-0 mt-0.5" />
                  <div>
                ...
                {Object.entries(ACCESSIBILITY_PRESETS).map(([key, preset]) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {selectedPreset === key && (
                        <Check className="w-5 h-5 text-laro flex-shrink-0" />
                      )}
                    </div>
                  </button>
                ))}
                  ...
                  <div className="flex gap-2">
                    {[
                      ...
                      >
                        ...
                        <span className={`text-sm font-medium ${theme === id ? 'text-laro' : ''}`}>
                          {label}
                        </span>
                      </button>
                    ))}
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {Object.entries(languages).slice(0, 6).map(([code, { name, flag }]) => (
                      ...
                      >
                        ...
                        <span className={`text-xs font-medium ${language === code ? 'text-laro' : 'text-muted-foreground'}`}>
                          {name}
                        </span>
                      </button>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {[
                      ...
                      >
                        <p className={`font-medium text-sm ${preferences.measurementUnit === id ? 'text-laro' : ''}`}>
                          {label}
                        </p>
