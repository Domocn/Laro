...
export const RecipeVersions = ({ recipeId, currentVersion, onRestore }) => {
  ...
  const handleRestore = async (version) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail || 'Couldn\'t restore that version. Please try again. (E-RV001)');
    } finally {
      setRestoring(null);
    }
  };
  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail || 'Couldn\'t compare those versions. Please try again. (E-RV002)');
    } finally {
      setComparing(false);
    }
  };
    ...
    <div className="bg-white dark:bg-card rounded-2xl border border-border/60 overflow-hidden" data-testid="recipe-versions">
      ...
      >
        <div className="flex items-center gap-2">
          <History className="w-5 h-5 text-laro" />
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          {version.version === currentVersion && (
                            <span className="px-2 py-0.5 bg-laro text-white text-xs rounded-full">
                              Current
                            </span>
                          )}
                        </div>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <GitCompare className="w-5 h-5 text-laro" />
              Version Comparison
            </DialogTitle>
          </DialogHeader>
