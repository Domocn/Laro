...
export const RecipeForm = () => {
  ...
  const loadRecipe = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Couldn\'t load that recipe. Please try again. (E-RF001)');
      navigate('/recipes');
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || (isEditing ? 'Couldn\'t update that recipe. Please try again.' : 'Couldn\'t create that recipe. Please try again.'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Couldn\'t upload that image. Please try again. (E-RF003)');
    } finally {
