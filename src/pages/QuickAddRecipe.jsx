...
export const QuickAddRecipe = () => {
  ...
  const handlePasteSubmit = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Couldn\'t parse that recipe. Please try again. (E-QA001)');
    } finally {
  ...
  const handleUrlSubmit = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Couldn\'t extract the recipe from that URL. Please try again. (E-QA002)');
    } finally {
      setUrlLoading(false);
    }
  };
  ...
  const handleImportSubmit = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Couldn\'t import those recipes. Please try again. (E-QA003)');
    } finally {
  ...
  const handleSaveRecipe = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Couldn\'t save that recipe. Please try again. (E-QA004)');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handlePasteFromClipboard = async () => {
    try {
      ...
      toast.error('Could not access clipboard (E-QA005)');
    }
  };
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {extractedRecipe.ingredients?.length > 6 && (
                      <span className="px-2 py-1 bg-laro-light text-laro rounded-lg text-xs">
