...
export const Dashboard = () => {
  ...
  useEffect(() => {
    if (user?.id) {
      const tourKey = `laro_feature_tour_seen_${user.id}`;
  ...
  const dismissTour = () => {
    if (user?.id) {
      localStorage.setItem(`laro_feature_tour_seen_${user.id}`, 'true');
    }
    setShowTour(false);
  };
  ...
  const loadData = async () => {
    try {
      ...
      toast.error(error.response?.data?.detail || 'Couldn\'t load your data. Please try again. (E-DS001)');
    } finally {
      setLoading(false);
    }
  };
  ...
  const quickActions = [
    ...
    { icon: CalendarDays, label: 'Meal Plan', path: '/meal-planner', color: 'bg-laro', shadowColor: 'shadow-card' },
      ...
      <div className="space-y-10" data-testid="dashboard">
        ...
        >
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <Link to="/recipes/new">
              <Button className="rounded-full bg-laro hover:bg-laro-dark shadow-sm" data-testid="dashboard-add-recipe">
          ...
          {quickActions.map((action, index) => {
            ...
            return (
              <Link key={action.label} to={action.path}>
                <div className="group p-4 sm:p-6 rounded-2xl bg-white border border-border/60 shadow-soft hover:shadow-hover transition-all duration-300 hover:-translate-y-1">
