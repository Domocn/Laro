...
export const SharedRecipe = () => {
  ...
  const handleCopyLink = async () => {
    ...
    try {
      ...
      toast.error('Couldn\'t copy the link. Please try again. (E-SR001)');
    }
  };
    ...
    return (
      <div className="min-h-screen bg-cream dark:bg-background flex items-center justify-center">
        <div className="text-center">
          <Loader2 className="w-10 h-10 animate-spin text-laro mx-auto mb-4" />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-cream to-cream-subtle dark:from-background dark:to-muted/20">
      ...
      <header className="bg-white/80 dark:bg-card/80 backdrop-blur-md border-b border-border/40 sticky top-0 z-40 print:hidden">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          <Link to="/" className="flex items-center hover:opacity-80 transition-opacity">
            <img src="/laro-banner.png" alt="Laro" className="h-9" />
          </Link>
      ...
      <main className="max-w-lg mx-auto px-4 py-6 sm:py-10">
        ...
        >
          {/* Hero Image */}
          <div className="relative aspect-[4/3] overflow-hidden bg-gradient-to-br from-laro/20 to-laro/5">
            {recipe.image_url ? (
              <img
          ...
          <div className="grid grid-cols-3 border-b border-border/40">
            ...
            <div className="flex flex-col items-center justify-center py-4">
              <Users className="w-5 h-5 text-laro mb-1" />
            ...
            <div className="px-5 py-3 border-b border-border/40">
              <div className="flex flex-wrap gap-2">
                {recipe.tags.map((tag, index) => (
                  <span
                    key={`${tag}-${index}`}
                    className="px-3 py-1 bg-laro/10 text-laro text-xs font-medium rounded-full"
                  >
                    {tag}
                  </span>
                ))}
          ...
          <div className="p-5 space-y-3 print:hidden">
            ...
            <Button
              ...
              className="w-full rounded-xl text-laro hover:text-laro-dark hover:bg-laro/5"
            >
              {showFullRecipe ? (
                <>
                ...
                <div className="p-5 space-y-6">
                  ...
                  <section>
                    <h2 className="font-heading text-lg font-semibold mb-3 flex items-center gap-2">
                      <span className="w-7 h-7 bg-laro/10 rounded-lg flex items-center justify-center text-laro text-sm">📝</span>
                      Ingredients
                    </h2>
                    ...
                    <ol className="space-y-4">
                      {recipe.instructions?.map((step, index) => (
                        <li key={`instruction-${index}`} className="flex gap-3">
                          <span className="w-6 h-6 bg-laro text-white rounded-full flex-shrink-0 flex items-center justify-center text-sm font-medium">
                            {index + 1}
                          </span>
        ...
        <motion.div
          ...
          className="mt-6 bg-gradient-to-br from-laro to-laro-dark rounded-2xl p-6 text-white text-center print:hidden"
        >
        ...
        <div className="mt-6 text-center text-xs text-muted-foreground print:hidden">
          <p>Shared via <Link to="/" className="text-laro hover:underline">Laro</Link> - Your Personal Recipe Manager</p>
        </div>
      </main>
