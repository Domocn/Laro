...
export const ImportRecipe = () => {
  ...
  // Pre-fill URL from query parameter and auto-extract
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      const recipeData = {
        ...
        prep_time: parseInt(extractedRecipe.prep_time) || 0,
        cook_time: parseInt(extractedRecipe.cook_time) || 0,
      ...
      toast.error(error.response?.data?.detail || 'Couldn\'t save the recipe. Please try again. (E-IR002)');
    } finally {
      setSaving(false);
    }
  };
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {extractedRecipe.ingredients?.slice(0, 5).map((ing, idx) => (
                      <span key={`${ing.name}-${idx}`} className="px-2 py-1 bg-white rounded-lg text-sm">
                        {ing.amount} {ing.unit} {ing.name}
                      </span>
                    ))}
                    {extractedRecipe.ingredients?.length > 5 && (
                      <span className="px-2 py-1 bg-laro-light text-laro rounded-lg text-sm">
                ...
                {extractedRecipe.tags && extractedRecipe.tags.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {extractedRecipe.tags.map((tag, idx) => (
                      <span key={`${tag}-${idx}`} className="px-2 py-1 bg-laro-light text-laro rounded-full text-xs">
                        {tag}
                      </span>
                    ))}
                  </div>
