...
export const Settings = () => {
  ...
  const handleTestLlm = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Couldn\'t test the connection. Please try again. (E-ST001)');
    } finally {
      setTestingLlm(false);
    }
  };
  ...
  const handleSavePrompt = async (promptType) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Couldn\'t save the prompt. Please try again. (E-ST006)');
    } finally {
  ...
  const handleResetPrompt = async (promptType) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Couldn\'t reset the prompt. Please try again. (E-ST007)');
    }
  };
  ...
  const handleResetAllPrompts = async () => {
    try {
      ...
      toast.error(error.response?.data?.detail || 'Couldn\'t reset prompts. Please try again. (E-ST008)');
    }
  };
  ...
  const handleImportFile = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Couldn\'t import those recipes. Please try again. (E-ST014)');
    } finally {
      setImporting(false);
      e.target.value = '';
    }
  ...
  const handleDownloadMyData = async () => {
    ...
    try {
      ...
      const downloadUrl = exportApi.gdprDownloadUrl();

      // Create a link and trigger download with auth header
      ...
      a.href = url;
      a.download = `laro_data_export_${new Date().toISOString().split('T')[0]}.zip`;
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Couldn\'t delete your account. Please try again. (E-ST016)');
    } finally {
      setDeleting(false);
    }
  };
  ...
  const handleCreateToken = async () => {
    ...
    try {
      const expiryDays = newTokenExpiry === 'never' ? null : parseInt(newTokenExpiry);
  ...
  useEffect(() => {
    if (activeTab === 'user' && apiTokens.length === 0) {
      loadApiTokens();
    }
  }, [activeTab]);
  ...
  const currentServer = localStorage.getItem('laro_server_url') || process.env.REACT_APP_BACKEND_URL;
    ...
    <Layout>
      <div className="max-w-2xl mx-auto space-y-8" data-testid="settings-page">
        ...
        >
          ...
          <div className="p-4 space-y-4">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-full bg-laro/20 flex items-center justify-center text-laro text-xl font-semibold">
          ...
          <div className="p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                {darkMode ? (
                  <Moon className="w-5 h-5 text-laro" />
                ) : (
          ...
          <div className="p-4 space-y-4">
            ...
            <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-border/60 rounded-xl cursor-pointer hover:bg-cream-subtle transition-colors">
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
                {importing ? (
                  <Loader2 className="w-8 h-8 text-laro animate-spin mb-2" />
                ) : (
                  <Upload className="w-8 h-8 text-muted-foreground mb-2" />
                )}
                <p className="text-sm text-muted-foreground">
                  <span className="font-medium text-laro">Upload a file</span> or drag and drop
                </p>
        ...
        >
          ...
          <div className="p-4 space-y-4">
            <p className="text-sm text-muted-foreground">
              Create API tokens for integrations like Home Assistant. Tokens provide access to your account without sharing your password.
            </p>

            {/* Newly created token display */}
            {newlyCreatedToken && (
              <div className="p-4 bg-laro-light rounded-xl border border-laro/30">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="outline" size="icon" onClick={() => copyToken(newlyCreatedToken)} className="rounded-lg flex-shrink-0">
                    ...
                    <Button
                      ...
                      className="text-laro/70 hover:text-laro"
                    >
                      I've saved my token
                    </Button>
                  </div>
            ...
            {loadingTokens ? (
              ...
              <div className="space-y-2">
                <Label className="text-sm text-muted-foreground">Your Tokens</Label>
                <div className="divide-y divide-border/60 border border-border/60 rounded-xl overflow-hidden">
                  {apiTokens.map((token) => (
                    <div key={token.id} className="p-3 flex items-center justify-between bg-white dark:bg-card">
              ...
              <p className="text-sm text-muted-foreground text-center py-4">
                No API tokens yet. Create one to integrate with external services.
              </p>
            )}

            <div className="p-3 bg-amber-50 rounded-xl border border-amber-200">
              <p className="text-xs text-amber-800">
                <strong>Security tip:</strong> Only create tokens for services you trust. Tokens have full access to your account.
        ...
        >
          ...
          <div className="p-4 space-y-4">
            <p className="text-sm text-muted-foreground">
              Download a complete copy of all your personal data stored in Laro. This includes your profile, recipes, cookbooks, pantry items, meal plans, and shopping lists.
            </p>

            <div className="p-3 bg-laro-light rounded-xl border border-laro/30">
              <p className="text-sm text-laro">
                <strong>Your data belongs to you.</strong> This export complies with GDPR Article 20 (Right to Data Portability).
              </p>
            </div>
            ...
            {serverInfo?.is_cloud ? (
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <button
                    ...
                    className={`p-4 rounded-xl border-2 text-left transition-all ${
                      llmSettings.provider === 'openai'
                        ? 'border-laro bg-laro-light'
                        : 'border-border/60 hover:border-laro'
                    }`}
                  >
                    <div className="flex flex-col items-center text-center gap-2">
                      <Cloud className={`w-6 h-6 ${llmSettings.provider === 'openai' ? 'text-laro' : 'text-muted-foreground'}`} />
                      <div>
                  ...
                  >
                    <div className="flex flex-col items-center text-center gap-2">
                      <Sparkles className={`w-6 h-6 ${llmSettings.provider === 'anthropic' ? 'text-laro' : 'text-muted-foreground'}`} />
                      <div>
                  ...
                  >
                    <div className="flex flex-col items-center text-center gap-2">
                      <HardDrive className={`w-6 h-6 ${llmSettings.provider === 'ollama' ? 'text-laro' : 'text-muted-foreground'}`} />
                      <div>
                  ...
                  >
                    <div className="flex flex-col items-center text-center gap-2">
                      <WifiOff className={`w-6 h-6 ${llmSettings.provider === 'embedded' ? 'text-laro' : 'text-muted-foreground'}`} />
                      <div>
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    ...
                    <a 
                      ...
                      className="text-laro hover:underline"
                    >
                ...
                <div className="pt-2">
                  ...
                  {llmTestResult && (
                    <div className={`mt-3 p-3 rounded-lg flex items-center gap-2 ${
                      llmTestResult.success ? 'bg-laro-light text-laro' : 'bg-red-50 text-red-600'
                    }`}>
            ...
            {Object.entries(promptLabels).map(([key, label]) => (
              <div key={key} className="border border-border/60 rounded-xl overflow-hidden">
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    {customPrompts[key] && (
                      <span className="text-xs bg-laro/20 text-laro px-2 py-0.5 rounded-full">
                        Customized
                      </span>
                    )}
                ...
                {editingPrompt === key && (
                  ...
                  >
                    <textarea
                      ...
                      className="w-full h-48 p-3 text-sm border border-border/60 rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-laro/50 font-mono"
                    />
                    <div className="flex items-center justify-between">
                      <Button
                        variant="ghost"
          ...
          <div className="divide-y divide-border/60">
            ...
            <div className="p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  {isLocalServer ? (
                    <Wifi className="w-5 h-5 text-laro" />
                  ) : (
              ...
              <div className="divide-y divide-border/60">
                {householdMembers.map((member) => (
                  <div key={member.id} className="p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-laro/20 flex items-center justify-center text-laro text-sm font-medium">
                        {member.name?.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) || 'U'}
                      </div>
                      <div>
                        <p className="font-medium text-sm">{member.name}</p>
                        {member.id === user?.id && (
                          <span className="text-xs text-laro">You</span>
                        )}
                      </div>
                    </div>
