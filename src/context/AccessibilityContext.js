...
export const AccessibilityProvider = ({ children }) => {
  ...
  const getInitialValue = (key, defaultValue) => {
    const saved = localStorage.getItem(`laro_${key}`);
  ...
  // Apply dyslexic font
  useEffect(() => {
    localStorage.setItem('laro_dyslexic_font', dyslexicFont.toString());
  ...
  // Apply text spacing
  useEffect(() => {
    localStorage.setItem('laro_text_spacing', textSpacing);
    const root = document.documentElement;
  ...
  // Apply line height
  useEffect(() => {
    localStorage.setItem('laro_line_height', lineHeight);
    const root = document.documentElement;
  ...
  // Apply focus mode
  useEffect(() => {
    localStorage.setItem('laro_focus_mode', focusMode.toString());
    const root = document.documentElement;
  ...
  useEffect(() => {
    localStorage.setItem('laro_simplified_mode', simplifiedMode.toString());
    const root = document.documentElement;
  ...
  useEffect(() => {
    localStorage.setItem('laro_contrast_level', contrastLevel);
    const root = document.documentElement;
  ...
  useEffect(() => {
    localStorage.setItem('laro_animation_level', animationLevel);
    const root = document.documentElement;
  ...
  useEffect(() => {
    localStorage.setItem('laro_highlight_step', highlightCurrentStep.toString());
  }, [highlightCurrentStep]);

  useEffect(() => {
    localStorage.setItem('laro_progress_indicators', showProgressIndicators.toString());
  }, [showProgressIndicators]);

  useEffect(() => {
    localStorage.setItem('laro_confirm_actions', confirmActions.toString());
  }, [confirmActions]);

  useEffect(() => {
    localStorage.setItem('laro_icon_labels', iconLabels.toString());
  }, [iconLabels]);

  useEffect(() => {
    localStorage.setItem('laro_reading_ruler', readingRuler.toString());
  }, [readingRuler]);

  useEffect(() => {
    localStorage.setItem('laro_sound_effects', soundEffects.toString());
  }, [soundEffects]);

  useEffect(() => {
    localStorage.setItem('laro_haptic_feedback', hapticFeedback.toString());
  }, [hapticFeedback]);

  useEffect(() => {
    localStorage.setItem('laro_timer_notifications', timerNotifications);
  }, [timerNotifications]);
