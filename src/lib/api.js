...
api.interceptors.request.use((config) => {
  ...
  if (config.method === 'get' && config.url) {
    ...
    if (cached) {
      // Return cached response by throwing a special object
...
export const recipeApi = {
  ...
  // Personal rating
  getRating: (id) => api.get(`/recipes/${id}/rating`),
...
export const adminApi = {
  ...
  sendTestNotification: (data) => api.post('/admin/notifications/test', data),
  sendCustomNotification: (data) => api.post('/admin/notifications/send-custom', data),
...
export const pantryApi = {
  ...
  getStaples: () => api.get('/pantry/staples'),
  getAll: (params) => api.get('/pantry', { params }),
  getExpiring: (days = 7) => api.get('/pantry/expiring', { params: { days } }),
  get: (itemId) => api.get(`/pantry/${itemId}`),
  create: (data) => api.post('/pantry', data),
  bulkCreate: (items) => api.post('/pantry/bulk', { items }),
  update: (itemId, data) => api.put(`/pantry/${itemId}`, data),
  delete: (itemId) => api.delete(`/pantry/${itemId}`),
  bulkDelete: (itemIds) => api.delete('/pantry/bulk/clear', { data: { item_ids: itemIds } }),
  matchRecipes: (data) => api.post('/pantry/match', data),
};

// Cookbooks APIs
export const cookbooksApi = {
  getAll: (search) => api.get('/cookbooks', { params: search ? { search } : {} }),
  get: (cookbookId) => api.get(`/cookbooks/${cookbookId}`),
...
export const exportApi = {
  ...
  download: (exportId) => `${getApiBaseUrl()}/export/download/${exportId}`,
  // GDPR Data Export - returns URL for direct download
  gdprDownloadUrl: () => `${getApiBaseUrl()}/export/gdpr`,
};
