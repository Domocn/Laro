...
export function useLiveRefresh(options = {}) {
  const {
    ...
    maxReconnectAttempts = 30, // 5+ minutes of retry with exponential backoff
  } = options;
      ...
      ws.onclose = (event) => {
        ...
        if (event.code !== 1000 && event.code !== 4001) {
          if (reconnectAttemptsRef.current < maxReconnectAttempts) {
            ...
            // Exponential backoff with jitter for mobile network resilience
            const baseDelay = Math.min(
              initialReconnectInterval * Math.pow(2, reconnectAttemptsRef.current - 1),
            ...
            console.log(`LiveRefresh: Reconnecting in ${delay}ms (attempt ${reconnectAttemptsRef.current})`);
  ...
  }, [maxReconnectAttempts, initialReconnectInterval, maxReconnectInterval, send, notifyListeners]);
